...
public final class ToastUtils {
   ...
   // private static int gravity         = Gravity.CENTER_HORIZONTAL | Gravity.BOTTOM;
    ...
    private static int yOffset         = (int) (64 * App.Companion.getInstance().getResources().getDisplayMetrics().density + 0.5);
    ...
    public static void setView(@LayoutRes int layoutId) {
        LayoutInflater inflate = (LayoutInflater) App.Companion.getInstance().getSystemService(Context.LAYOUT_INFLATER_SERVICE);
     ...
     * @param text 文本
     */
    public static void showShortSafe(final String text,long delay) {
    ...
    private static void show(@StringRes int resId, int duration) {
        show(App.Companion.getInstance().getResources().getText(resId).toString(), duration);
    }
    ...
    private static void show(@StringRes int resId, int duration, Object... args) {
        show(String.format(App.Companion.getInstance().getResources().getString(resId), args), duration);
    }
    ...
    private static void show(CharSequence text, int duration) {
        ...
        if (sViewWeakReference != null) {
            final View view = sViewWeakReference.get();
            if (view != null) {
                sToast = new Toast(App.Companion.getInstance());
                sToast.setView(view);
        ...
        if (!isCustom) {
            if (messageColor != DEFAULT_COLOR) {
                ...
                sToast = Toast.makeText(App.Companion.getInstance(), spannableString, duration);
            } else {
                sToast = Toast.makeText(App.Companion.getInstance(), text, duration);
            }
        }
